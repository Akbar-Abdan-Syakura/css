...
<form action="" method="post">
  <header class="navbar navbar-dark sticky-top bg-dark flex-md-nowrap p-0 shadow">
    ...
    <input type="text" class="form-control form-control-dark w-100 rounded-0 border-0" placeholder="Cari Kondisi Aset" name="textcari">
    ...
    <div class="row">
      <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
        <div class="position-sticky pt-3">
          <ul class="nav flex-column">
            ...
            <li class="nav-item">
              <a class="nav-link active" href="monitoring_procurement.php?page=1">
      ...
      <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
          <h1 class="h2">Daftar Kondisi Aset</h1>
          <div class="btn-toolbar mb-2 mb-md-0">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb">
                <li class="breadcrumb-item active">Dashboard</a></li>
                <li class="breadcrumb-item active">Kondisi Aset</a></li>
              </ol>
            </nav>
              ...
              <table class="table table-bordered">
                ...
                foreach ($aset as $ast) : ?>
                  <tbody>
                    <tr>
                      <td><?= $no; ?></td>
                      <td><?= sprintf("%03d", $ast["id_aset"]); ?><?= sprintf("%03d", $ast["id_divisi"]); ?><?= date('Y', strtotime($ast["tanggal_masuk"])); ?></td>
                      <td><?= $ast["nama_aset"]; ?></td>
                      <td><?= $ast["merk"]; ?></td>
                      <td><?= $ast["type"]; ?></td>
                      <td><?= $ast["qty"]; ?> <?= $ast["satuan"]; ?></td>
                      <td><?= $ast["nama_divisi"]; ?></td>
                      <td><?= $ast["tanggal_masuk"]; ?></td>
                      <?php
                         if ($ast["id_kondisi_aset"] == 4){
                    ?>
                    <td class="table-danger text-center"><?= $ast["kondisi"]; ?></td>
                    
                    <?php } else if ($ast["id_kondisi_aset"] == 3){ ?>
                        
                        <td class="table-warning text-center"><?= $ast["kondisi"]; ?></td>
                    ...
                    <?php } else { ?>
                     ...
                     <?php } ?>
                      <td class="text-center">
                        <a class="btn btn-outline-warning" role="button" href="detail_aset_procurement.php?id_aset=<?= $ast["id_aset"]; ?>"><i class="bi bi-list"></i></a>
                       </td>
                    </tr>
                    <?php $no++; ?>
