...
$jumlahdata = 10;
$jumlahuser = count(tampil_user("SELECT * FROM users"));
...
$users = tampil_user("SELECT * FROM users INNER JOIN divisi ON users.id_divisi = divisi.id_divisi LIMIT $awaldata, $jumlahdata");

if (isset($_POST["textcari"])) {

  $users = cari_user($_POST["textcari"]);
}
?>
<!doctype html>
<html lang="en">
...
<body>
  <form action="" method="post">
    <header class="navbar navbar-dark sticky-top bg-dark flex-md-nowrap p-0 shadow">
      ...
      <input type="text" class="form-control form-control-dark w-100 rounded-0 border-0" placeholder="Cari User" name="textcari">
    ...
    <div class="container-fluid">
      <div class="row">
        <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
          <div class="position-sticky pt-3">
            <ul class="nav flex-column">
              ...
              <li class="nav-item">
                <a class="nav-link" href="monitoring_admin.php">
                  <span data-feather="monitor" class="align-text-bottom"></span>
                  Kondisi Aset
                </a>
              </li>
              <li class="nav-item">
              <a class="nav-link" href="rekomendasi_requester.php">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
          ...
          <a class="btn btn-outline-primary" role="button" href="tambahuser.php"><i class="bi bi-plus-lg"></i> Tambah User</a>
          <p>
          ...
          <nav aria-label="Page navigation example">
            <ul class="pagination justify-content-center">
              <?php if ($halaktif > 1) : ?>
                <li class="page-item">
                  <a class="page-link" href="?page=<?= $halaktif - 1; ?>">Back</a>
                </li>
              <?php else : ?>
            ...
            <table class="table table-bordered">
              <thead>
                <tr>
                  ...
                  <th class="text-center" scope="col">Kode User</th>
              ...
              foreach ($users as $usr) : ?>
                <tbody>
                  <tr>
                    ...
                    <td class="text-center"><?= sprintf("%03d", $usr["id_users"]); ?></td>
                    <td class="text-center"><?= $usr["nama"]; ?></td>
                    <td class="text-center"><?= $usr["nama_divisi"]; ?></td>
                    <td class="text-center"><?= $usr["role"]; ?></td>
                    <td class="text-center">
                    <a class="btn btn-outline-warning" role="button" href="detailuser.php?id_user=<?= $usr["id_users"]; ?>"><i class="bi bi-list"></i></a>
                    <a class="btn btn-outline-success" role="button" href="ubahuser.php?id_user=<?= $usr["id_users"]; ?>"><i class="bi bi-pencil-square"></i></a>
                      <button type="button" class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deleteModal<?= $usr['id_users']; ?>"><i class="bi bi-trash3-fill"></i></button>
                    </td>
                  </tr>
                  <?php $no++; ?>
                  <!-- Modal Delete -->
                  <div class="modal fade" id="deleteModal<?= $usr['id_users']; ?>"" tabindex=" -1" aria-labelledby="hapusUsers" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="hapusUser">Hapus User </h5>
                          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                          Yakin akan Menghapus User <b><?= $usr["nama"]; ?></b> ?
                        </div>
                        <div class="modal-footer">
                          
                          <a class="btn btn-outline-danger" href="hapususer.php?id=<?= $usr["id_users"]; ?>"><i class='bi bi-trash3-fill'></i> Hapus</a>
